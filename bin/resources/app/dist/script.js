...
// Launcher Auto Update Toggle
btnAutoupdate.addEventListener('click', () => ipcRenderer.send('toggle-launcher-autoupdate'));
ipcRenderer.send('get-launcher-autoupdate-state');
...
ipcRenderer.on('launcher-autoupdate-state', (event, isEnabled) => {
  btnAutoupdate.style.opacity = isEnabled ? '1' : '0.4';
  btnAutoupdate.title = isEnabled ? 'Launcher Auto-Update: ON' : 'Launcher Auto-Update: OFF';
});
