...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$WshShell = New-Object -ComObject WScript.Shell; $StartMenu = [System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs'); $Shortcut = $WshShell.CreateShortcut(\"$StartMenu\Madrilla Launcher.lnk\"); $Shortcut.TargetPath = \"$PSScriptRoot\MadrillaLauncher.exe\"; $Shortcut.WorkingDirectory = \"$PSScriptRoot\"; $Shortcut.Save()"

:: Create Desktop Shortcut
powershell -NoProfile -ExecutionPolicy Bypass -Command "$WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut(\"$Home\Desktop\Madrilla Launcher.lnk\"); $Shortcut.TargetPath = \"$PSScriptRoot\MadrillaLauncher.exe\"; $Shortcut.WorkingDirectory = \"$PSScriptRoot\"; $Shortcut.Save()"
